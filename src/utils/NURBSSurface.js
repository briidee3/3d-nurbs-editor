...
const defaultNurbsParams = {
  "ctrlPts": [
    ...
    [
      ...
      {
        x: -53.6635752031526,
        y: 11.077541484296924,
        z: 0,
        w: 1
      },
      {
        x: -53.598444182247306,
        y: 14.112879767155514,
        z: 0,
        w: 1
      },
      {
        x: -39.91363025089749,
        y: 28.466947741819578,
        z: 0,
        w: 1
      },
      {
        x: -36.464249416984075,
        y: 17.49999999999998,
        z: 0,
        w: 1
      }
    ...
    [
      ...
      {
        x: -32.87079457434081,
        y: 18.904827582352222,
        z: 0,
        w: 1
      },
      {
        x: -23.687427933176075,
        y: 17.500000000000014,
        z: 0,
        w: 1
      }
    ],
    [
      {
        x: 1.4924993132530912e-14,
        y: 0,
        z: 0,
        w: 1
      },
      {
        x: -4.9657935401588323e-14,
        y: 6.464829954221287,
        z: 0,
        w: 1
      },
      {
        x: 8.031798585909324e-14,
        y: 15.051730024658026,
        z: 0,
        w: 1
      },
      {
        x: -2.7705891254088542e-14,
        y: 4.1951520742499495,
        z: 0,
        w: 1
      },
      {
    ...
    [
      {
        x: 54.802940837264494,
        y: 0,
        z: 0,
        w: 1
      },
      {
        x: 53.663575203152526,
        y: 11.077541484296937,
        z: 0,
        w: 1
      },
      {
...
/**
 * Simple alternative method of point inversion. Basically a weird version of gradient descent. Assumes no local minima.
 * @param {*} tol - Tolerance
 * @param {*} maxIt - Max number of iterations
 * @param {*} pVec - Point to approximate in vector form
 * @param {*} surfaceObject - SurfaceObject instance
//  * @param {*} nurbsPos - Center point of the NURBS surface in xy-space
 */
function pointInversion(tol, maxIt, pVec, surfaceObject, uOffset = 0, vOffset = 0) {
  const initMod = 0.4;
  var curDiff, curU = 0.5 + uOffset, curV = 0.5 + vOffset, curIt = 0, curModU = initMod + (Math.abs(uOffset) / 2), curModV = initMod + (Math.abs(vOffset) / 2), i, curMin = 999999999, curMinIndex;
  const curPts = [
    new Vector3(),
    new Vector3(),
    new Vector3(),
  ...
  const p = new Vector3(pVec.x - surfaceObject.nurbsObj.position.x, pVec.y - surfaceObject.nurbsObj.position.y, 0);

  while (curMin > tol && curIt < maxIt) {
    // Get next set of points
    surfaceObject.getPoint(curU + curModU, curV + curModV, curPts[0]);
    surfaceObject.getPoint(curU + curModU, curV - curModV, curPts[1]);
    surfaceObject.getPoint(curU - curModU, curV - curModV, curPts[2]);
    surfaceObject.getPoint(curU - curModU, curV + curModV, curPts[3]);
    // surfaceObject.getPoint(curU - curModU, curV, curPts[4]);
    // surfaceObject.getPoint(curU + curModU, curV, curPts[5]);
    // surfaceObject.getPoint(curU, curV - curModV, curPts[6]);
    // surfaceObject.getPoint(curU, curV + curModV, curPts[7]);

    curMinIndex = 0; // default to the first one => only need to check the following 3
    curMin = (curPts[0].x - p.x) ** 2 + (curPts[0].y - p.y) ** 2;
    for (i = 1; i < curPts.length; i++) {
      curDiff = (curPts[i].x - p.x) ** 2 + (curPts[i].y - p.y) ** 2;
      if (curDiff < curMin) {
        curMin = curDiff;
        curMinIndex = i;
      }
    ...
    curU = Math.max(Math.min(1, curU), 0);
    curV = Math.max(Math.min(1, curV), 0);
  }

  return { x: curU, y: curV, isUv: true };
}

function pointInversionXYZ(dn, tol, maxIt, p, damp, surfaceObject, dim = 2, asArray = false) {
    // Use new version
    const out = pointInversion(tol, maxIt, p, surfaceObject);
    console.log({...out})
    return out;

    // const z = p.z - surfaceObject.nurbsObj.position.z || -surfaceObject.z;
    // const pVec = new Vector3(p.x - surfaceObject.nurbsObj.position.x, p.y - surfaceObject.nurbsObj.position.y, 0);
    // console.log(pVec)
    // const result = calcNearestSurfacePointFromPoint(dn, tol, maxIt, pVec, damp, surfaceObject);
    // const dimChars = [ 'x', 'y', 'z', 'w'];
    // // console.log(result);
    // // const test = new Vector3();
    // // surfaceObject.getPoint(result[0], result[1], test);
    // // const diff = {
    // //   x: result[0] + surfaceObject.nurbsObj.position.x - p.x,
    // //   y: result[1] + surfaceObject.nurbsObj.position.y - p.y
    // // };
    // // console.log(`Diff between acquired and expected: (${diff.x}, ${diff.y})`);
    // // console.log(test)
    // // console.log(p)
...
function nurbsCurveApprox(points_, deg, num_cpts, weights) {
    ...
    const N = [];
    computeN(deg, num_cpts, points.length, N, U, uk);
    ...
    for (i = 1; i < points.length - 1; i++) {
        ...
        n0p = calcBasisFuncOne(deg, U, points.length - 1, uk[i]);
        elem2 = [];
        elem3 = [];
        tmp = [];
        for (j = 0; j < dim; j++) {
            ...
            tmp.push(points[i][j] - pt0[j] * n0p - ptm[j] * nnp);
        }
        rk.push(tmp);
    }
    ...
    for (i = 1; i < num_cpts - 1; i++) {
        ...
        for (j = 0; j < rk.length; j++) {
            tmp = []
            for (k = 0; k < dim; k++) {
                tmp.push(rk[j][k] * calcBasisFuncOne(deg, U, i, uk[j + 1]));
            }
            ru_tmp.push(tmp);
        }
        for (j = 0; j < dim; j++) {
    ...
    for (i = 0; i < dim; i++) {
        ...
        solveLURow(NTN, tmp, sol[j], NTN.length, indx);
        // console.log(tmp_1)
...
function curvesIntersection(degU, U, ctrlPtsU, degV, V, ctrlPtsV, maxIt) {
    var uOut, vOut, i, j;
    const uTol = 0.3, vTol = 0.3;

    var curPtU, curPtV, curU = 0.5, curV = 0.5;
    var uStepSize = 0.5, vStepSize = 0.5;
    var uPts, vPts, dists;
    var curIt = 0;

    while (curIt < maxIt && ((curPtU.x - curPtV.x) ** 2 > uTol || (curPtU.y - curPtV.y) ** 2 > vTol)) {
        uPts = [
            calcBSplinePoint(degU, U, ctrlPtsU, curU - uStepSize),
            calcBSplinePoint(degU, U, ctrlPtsU, curU + uStepSize),
        ];
        vPts = [
            calcBSplinePoint(degV, V, ctrlPtsV, curV - vStepSize),
            calcBSplinePoint(degV, V, ctrlPtsV, curV + vStepSize)
        ];
